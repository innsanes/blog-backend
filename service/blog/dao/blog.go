...
func Create(m *model.Blog) (err error) {
	err = g.MySQL.Model(&model.Blog{}).Create(m).Error
	return
}

func Update(m *model.Blog) (err error) {
	err = g.MySQL.Model(&model.Blog{}).Where("id = ?", m.ID).Updates(m).Error
	return
}

func Get(id uint) (t *model.Blog, err error) {
	err = g.MySQL.Model(&model.Blog{}).Where("id = ?", id).Find(&t).Error
	return
}
